...
`|` - или (перечисления), хотя бы одно

`()` - группирующие сохраняющие скобки (каждая отдельно и все вместе)

`(?:...)` - группа без захвата (часть общего, не ищется отдельно)
...
## Позиция в тексте (а не текст)

### Простые шаблоны

`^` - начало всего текста (с флагом `re.MULTILINE` – начало строки)

`$` - конец всего текста (с флагом `re.MULTILINE` – позиция перед символом переноса строки `\n`)
...
![позиция в тексте](img/borders-01.png)

### Сложные шаблоны

`(?=...)` - *lookahead assertion (опережающая проверка)*, соответствует каждой позиции, сразу после которой начинается соответствие шаблону ...

`(?!...)` - *negative lookahead assertion (негативная опережающая проверка)*, соответствует каждой позиции, сразу после которой НЕ может начинаться шаблон ...

`(?<=...)` - *positive lookbehind assertion (позитивная ретроспективная проверка)*, соответствует каждой позиции, которой может заканчиваться шаблон ...


Длина шаблона должна быть фиксированной, то есть abc и a|b — это ОК, а a* и a{2,3} — нет

`(?<!...)` - *negative lookbehind assertion (негативная ретроспективная проверка)*, соответствует каждой позиции, которой НЕ может заканчиваться шаблон ...

![Сложные шаблоны lookaround](img/lookaround-01.png)
...
`re.search(pattern, string, flags)` – ищет **первое вхождение** или возвращает None

`re.finditer(pattern, string, flags)` – ищет **все непересекающиеся** вхождения, возвращает match-объект или ничего
...
`re.fullmatch(pattern, string, flags)` – подходит ли строка string целиком под шаблон pattern или возвращает None
...
`match.group(n)` или `match[n]` – сохраняющая группа (n - номер группы)

![объект re.Match](img/match-01.png)
...
match_1 = re.findall(r'[0-9a-fA-F]{2}(?:[:-][0-9a-fA-F]{2}){5}', text)
print(match_1)  # ['01:23:45:67:89:ab']  # находит только: 01:23:45:67:89:ab

match_2 = re.findall(r'[0-9a-fA-F]{2}([:-][0-9a-fA-F]{2}){5}', text)
print(match_2)  # [':ab']  # находит: 01:23:45:67:89:ab  и  :ab


text_2 = 'он шёл, он плыл, тот шёл, тот плыл'

match_3 = re.findall(r'(?:он|тот) (?:шёл|плыл)', text_2)
print(match_3)  # ['он шёл', 'он плыл', 'тот шёл', 'тот плыл']

match_4 = re.findall(r'(он|тот) (шёл|плыл)', text_2)
print(match_4)  # [('он', 'шёл'), ('он', 'плыл'), ('тот', 'шёл'), ('тот', 'плыл')]
```
...
![Текст от #START# до #END#](img/start-end.png)
...
match = re.findall(r"\d+(?=_(?!_))", text)
print(match)  # ['12']
```

![ровно одно подчёркивание после](img/one-underline.png)

Строка, в которой **нет boo** (нет такого символа, перед которым есть boo)
...
match = re.findall(r"^(?:(?!boo).)*?$", text, flags=re.MULTILINE)
print(match)  # ['a foo and', 'and others']
```

Строка, в которой **нет ни boo, ни foo** (нет такого символа, перед которым есть boo)
...
match = re.findall(r"^(?:(?!boo)(?!foo).)*?$", text, flags=re.MULTILINE)
print(match)  # ['and others']
```
...
Например: +7-71-75-621-382, 8-707-762-435-126.
